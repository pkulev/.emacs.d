...
* About
  ...
  Requires at least version 28.1 of Emacs, but lately I used only 30+, so it may not work on earlier versions.
...
- [[#navigation][Navigation]]
  ...
  - [[#avy][Avy]]
- [[#editing][Editing]]
  - [[#structural-editing--navigation][Structural editing & navigation]]
...
- [[#autocomplete][Autocomplete]]
  - [[#company][Company]]
  - [[#vertico][Vertico]]
    - [[#order-and-visuals][Order and visuals]]
    - [[#embark][Embark]]
    - [[#search][Search]]
...
- [[#programming-modes][Programming modes]]
  - [[#common][Common]]
    - [[#search-project-and-directory-files][Search project and directory files]]
    ...
    - [[#env-project-integration][.env project integration]]
...
- [[#pdf-and-other-documents][PDF and other documents]]
  - [[#doc-view-the-built-in-document-viewer][Doc View, the built-in document viewer]]
- [[#downloading][Downloading]]
  - [[#mentor---rtorrent-xmlrpc-client][mentor - rTorrent XMLRPC client]]
...
*** Order and visuals
   Savehist persists completion history between Emacs restarts.
   #+begin_src elisp
   ...
   Orderless adds more flexible completion styles.
   #+begin_src elisp
   ...
   Marginalia adds rich annotations in the minibuffer (docstrings, descriptions, access
   bits and mtime for files and projects.
   #+begin_src elisp
...
*** Embark
   Context-aware actions on completion candidates and things at point.
   #+begin_src elisp
     (use-package embark
       :ensure t
       :bind
...
*** Search
    Consult provides searching and navigation in various contexts.
    Unfortunately it leaves to user the boring burden of remapping all the things.
    #+begin_src elisp
      (use-package consult
        :ensure t
        :bind
...
* Programming modes
** Common
*** Search project and directory files
    Ripgrep is really fast and has a neat transient UI within Emacs.
...
**** Highlight todos
     It's useful to highlight markers like TODO and FIXME in commentaries and docstrings as well as
     an ability to jump on them back and forth.
...
*** Compilation
    Set ANSI color filter that will produce nice colored reports.
    #+begin_src elisp
      (use-package compile
        :hook
...
** Ocaml
   ...
   #+end_src
** Rust
   Requires =rust-analyzer= on =PATH= (e.g. =rustup component add rust-analyzer=).
   #+begin_src elisp
...
** Doc View, the built-in document viewer
   #+begin_src elisp
...
*** Org export
    ...
    *ox-jira* is great package that exports org buffer to JIRA format, that can be pasted
    ...
    I use *ox-gfm* for exporting README.org to README.md (GitHub flavored markdown) that
    renders nice on GitHub.
    #+begin_src elisp
      (use-package ox-gfm
        :ensure t
        :defer t)
