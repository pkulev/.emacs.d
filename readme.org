...
** Where it works
   ...
   |---------------+------------+----------------------------+------------|
   | Emacs/Dist    | Emacs 26.2 | Remacs 26.2                | Emacs 27.0 |
   |---------------+------------+----------------------------+------------|
   | Fedora 29     | works      | works (but expect freezes) | not tested |
   | Fedora 30     | works      | works (but expect freezes) | not tested |
   | Centos 7      | works      | not tested                 | not tested |
   | Gentoo        | works      | not tested                 | works      |
   | Ubuntu 16 WSL | works      | not tested                 | not tested |
   | Termux        | works      | not supported              | not tested |
   |---------------+------------+----------------------------+------------|
